...
export WAM_COND_CONCAT=${WAM_COND_CONCAT:-3}
export WAM_COND_SPATIAL=${WAM_COND_SPATIAL:-1}
export WAM_ACTMAP_CACHE=$RUNS/actmap_cache_3cam     # L3: 14-dim action channel-concat conditioning
export CKPT_DIR=$RUNS/train_out_t1e_local
...
while [ "$ATTEMPT" -lt 40 ]; do
  ATTEMPT=$((ATTEMPT+1))
  echo "[t1e-local] attempt $ATTEMPT $(date) COND_CONCAT=$WAM_COND_CONCAT steps=$STEPS save=$SAVE (resumes from latest ckpt)"
  "$VENV/bin/torchrun" --nnodes=1 --nproc_per_node=8 --master_addr=127.0.0.1 --master_port=29517 \
  ...
  if ls -d "$CKDIR"/iter_0000030* >/dev/null 2>&1; then
    echo "[t1e-local] reached iter_3000 — DONE $(date)"; break
  fi
  echo "[t1e-local] attempt $ATTEMPT exited (likely CPU-OOM); retrying in 15s..."; sleep 15
done
