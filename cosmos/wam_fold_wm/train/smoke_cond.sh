...
export CKPT_DIR=$RUNS/smoke_cond
export WAM_COND_CONCAT=${WAM_COND_CONCAT:-14}
...
echo "[smoke-cond] $(date) steps=$STEPS ckpt=$CKPT_DIR latent_cache=$WAM_WM_LATENT_CACHE"
"$VENV/bin/torchrun" --nnodes=1 --nproc_per_node=2 --master_addr=127.0.0.1 --master_port=29513 \
