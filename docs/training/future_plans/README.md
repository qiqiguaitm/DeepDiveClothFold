...
| 🔴 P0 | [**Mode B 回折冻结 根因逐级排查**](plans/pi05_v4_awbc_modeB_freeze_diagnosis_plan.md) | 🔴 双诱因(DCT / dagger迟疑起手数据)坐实;根因=06-16变点迟疑起手 | — | — | 逐天分析:dagger 从 06-16 起碎片+迟疑起手(起爆延迟0.5s vs 前0.1s),被AWBC标positive+部署always-positive→回折塌缩;剂量效应(§9)|
| 🔴 P0 | [**dagger clip 双向起爆点前裁 修冻结**](plans/dagger_launchpoint_trim_freeze_fix_plan.md) | ✅ H成立, 起爆点前裁是修法 (2026-07-14 真机确认, loss 0.70→0.003) | 8× GPU | — | dagger 迟疑边界段是冻结根因; 裁掉后不再冻结 |
| ⭐⭐ P1 | [**CRAVE value 打标 × 新 dagger(chunk-001) 验证**](plans/crave_value_dagger_validation_plan.md) | 📋 plan 已写, 待打标+训练| 8× GPU | — | CRAVE GRU(corr 0.975)给新 chunk-001 dagger(拼接完整 ep,387)+等量 base 打标→AWBC→真机:新格式是否天然不冻 |
...
| ⭐⭐ P1 | [**CRAVE polyline 标签 × 重训 KAI0 pi0-AE**](plans/crave_polyline_kai_ae_retrain_plan.md) | ⚠️ v1 (loss_action=0) dead-value 确诊; ✅ **v2 MT 版 (loss_action=1.0) config+数据就绪, 待提交** (config `ADVANTAGE_TORCH_CRAVE_POLY_MT` / `_MONO_MT`; 提交信息见 plan §8)| 8× GPU × 2 臂 | — | CRAVE polyline(去阶梯)标签 × multi-task AE. **根因**: value-only 把 backbone 饿死 → dead; **修法**: 加回 action flow-matching 辅助任务 (loss_action=1.0), 前人已在 config.py:1061 诊断. 数据 `crave_stage_poly` + `_mono` 复用 v1, 无需重建 |
| ⭐⭐ P1 | [**从数据学 Advantage:CRAVE 监督→蒸馏→ranking**](plans/crave_ae_distill_plan.md) | 📋 Phase0 数据就绪; Phase1 标签已被 polyline 版取代(见上行); Phase2(ranking)D1 后推进 | 8× GPU | — | CRAVE 零人工进度当监督。Phase1 回归蒸馏 AE-A/B vs AE-C(⚠️旧 H+norm01 标签,已由 [polyline 重训 plan](plans/crave_polyline_kai_ae_retrain_plan.md) 取代); D1 后 Phase2 = g_φ+margin-ranking(监督用 CRAVE-value 序非 raw-time / 冻结·放开档 / 变点加权 / 读 A=g(s')−g(s))。含决策点 D1·D2 + sanity 三件套 |
