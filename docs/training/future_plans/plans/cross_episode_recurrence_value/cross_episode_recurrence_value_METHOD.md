...
**② autonomy 真机 rollout**(3 轮 7676f,含两次真退步):CRAVE 两次回落到 ~0 + 每轮恢复、round3→**1.0**,负 advantage 集中在两个轮次边界(对齐真退步,仅 5%);pi0-AE 整体偏单调弱爬升、end **0.33**,退步信号弥散(45% 帧负)。形状相关 0.25。**CRAVE 的退步检测既稀疏又对齐真事件,AE 的退步信号被噪声淹没**——这正是 AWBC 需要 negative advantage 段时 CRAVE 的优势(无需"推理失败段"也能给可靠负信号)。

![autonomy 对比](../../../../visualization/cross_episode_recurrence_value/crave_vs_ae_autonomy.png)

> 说明:两图的 AE 值取自同一物理 episode 的 Stage-2 `absolute_value`(ep808 取 `A_smooth800_dagger_all_awbc`;autonomy 取离线 `autonomy_pi0ae.npy`),CRAVE 用 dagger demo 挖的同一 milestone 模型现场应用。脚本 `train_scripts/kai/data/crave_vs_ae_{ep808,autonomy}.py`。
