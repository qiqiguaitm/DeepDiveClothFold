...
### 3.3b 单 episode 对比:分布外样本上 CRAVE 反超监督 pi0-AE

§3.3 的聚合 MAE 是 pi0-AE 的**主场**(它拟合自己的训练目标)。换到**分布外样本**(dagger 纠错 episode、真机 rollout)逐 episode 看,优劣反转——监督 AE 欠读、退步信号失真,CRAVE 仍干净 0→1。**真值锚定**:对照帧确认任务已完成(衣物叠好)。

**① dagger ep808**(5917f,**末帧已叠好=完成**):CRAVE 0→**1.00**(单调 100%,退步帧仅 5%);pi0-AE `absolute_value` 压缩到 **0.27**(max 0.66,单调仅 50%,**47% 帧误标负 advantage**)。形状相关 Pearson 0.47/τ 0.21。零训练 CRAVE 正确达 1.0,监督 AE 在 dagger 分布外欠读。

![ep808 对比](../../../../visualization/cross_episode_recurrence_value/crave_vs_ae_ep808.png)

**② autonomy 真机 rollout**(3 轮 7676f,含两次真退步;CRAVE 用规范 vis0526 全集挖矿,与 §3.4 / `rollout_v24_sync` 一致):CRAVE round1→~0.65 → ~100s 回落到 0 → round2→~0.65 → ~175s 回落到 0 → round3→**1.0**,负 advantage 集中在两个轮次边界(对齐真退步,仅 7%);pi0-AE 整体偏单调弱爬升、end **0.33**,退步信号弥散(45% 帧负)。形状相关 Pearson **0.64**/τ 0.38。**CRAVE 的退步检测既稀疏又对齐真事件,AE 的退步信号被噪声淹没**——这正是 AWBC 需要 negative advantage 段时 CRAVE 的优势(无需"推理失败段"也能给可靠负信号)。

![autonomy 对比](../../../../visualization/cross_episode_recurrence_value/crave_vs_ae_autonomy.png)

> 说明:两图的 AE 值取自同一物理 episode 的 Stage-2 `absolute_value`(ep808 取 `A_smooth800_dagger_all_awbc`;autonomy 取离线 `autonomy_pi0ae.npy`)。CRAVE 按各自域的规范挖矿源现场应用——ep808(smooth800 域)用 smooth800-dagger 挖矿,autonomy(vis 域)用 vis0526 全集挖矿(与 §3.4 `rollout_v24_sync` 完全一致,故绿线即最终版 CRAVE)。脚本 `train_scripts/kai/data/crave_vs_ae_{ep808,autonomy}.py`,同步对齐视频见 `crave_vs_ae_sync_video.py`。
