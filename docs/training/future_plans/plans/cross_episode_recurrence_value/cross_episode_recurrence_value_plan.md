...
#### 4.6.5 连续 value 推导说明(公式 + 微例)

> 本节给出连续 value 输出的完整推导,供复现/理解。脚本:`tcc_e2e_dp_readout_ep2047.py`(读出核心)· `generic_continuous_generalize.py`(通用)· `render_3way_ep2047.py`(对账)。

**一句话**:对每一帧问"它最像参考演示里进度百分之几的那一刻",再用 DP 把整条 episode 的逐帧答案整理成一条平滑单调曲线。进度的"真值参照" = 参考 demo 的归一化时间(假设 demo 大致单调推进任务)。

**准备 — 参考库(进度字典)**:取 K=30 条已完成参考 demo,每帧过 TCC 编码网络 → 向量 z,并贴标签 = 它在自己 demo 内的归一化时间 t/T。得到几千个 (向量, 进度) 对。

**符号**:query 第 t 帧编码向量 q_t;进度轴离散成 NB=201 格(中心 `bin(b)=b/200`,b=0..200);参考库第 j 帧 (z_j, τ_j)。

**① 相似度场**(每帧对每个进度格的支持度):
```
s_t[b] = max_{ j : round(τ_j·200)=b }  cos(q_t, z_j)
```
即 q_t 与"归一化时间落在格 b 的参考帧"的最大余弦相似度;空格记 −∞。

**② 代价 + 逐帧归一化**(TCC 相似度场过平,差异仅 ~0.01,须归一否则 DP 塌):
```
...
```
第二项是核心:进度不能乱跳。瞬时混叠(几帧)想跳回早期进度要付 λ 代价被拉回 → **fold 凹口消除**;持续真回退(松手多帧)累积证据翻过 λ → **value 真下降保留**。这与离散 CRAVE 的 DP 同机制。

**④ 子格软期望(阶梯→连续)** —— DP 输出格索引(量化),在路径格 ±W(=8)邻域内按相似度 softmax 加权求期望:
```
v_t = Σ_{b ∈ [b_t−W, b_t+W]}  softmax_b( s_t[b]/0.03 ) · bin(b)
```
再过一道轻中值平滑(窗 9)去毛刺 → 输出 `v_t ∈ [0,1]`,逐帧连续单调。

**微例**(某帧"布摊开一半"):① 相似度场最像参考"进度 0.55"段(0.97),也略像早期 0.10(0.85);② 单帧 argmax 若被外观混叠误给 0.12;③ DP 见前一帧 v=0.53、后一帧也在 0.55 区,跳到 0.12 要付 λ·0.41 大代价、留 0.55 几乎不付 → **DP 选 0.55,误判被纠正**;④ 软期望在 0.55 附近细调 → v_t=0.558。

**与离散 CRAVE 的关系**:同特征(armmask⊕raw⊕proprio)、同"DP 时序证据"思想;离散把帧硬分到 milestone 簇、value=已过 milestone 数(阶梯),连续把帧软对齐到连续时间轴(参考库)、DP+软期望出平滑曲线。**λ 与 NB 是密度↔鲁棒/粒度旋钮**(NB=21 阶梯、201 连续、1001 最细;λ 小保密度、大更单调)。
