...
图像目录:`docs/visualization/cross_episode_recurrence_value/`(本文图 1-21 均相对引用,GitHub 直接渲染);视频不入 git,路径见附录 A。
...
| 4 | ✅ value 是状态触发,可迁移真机 rollout | demo 挖掘 → rollout 重试处回落 | 图7/8 |
...
> 诚实标注:episode 内高 τ 含"时间单调"成分;**鉴别性证据在 (d)**——rollout 重试处回落,时间线性信号做不到。
...
#### (d) 鉴别性实验:demo 挖掘的 milestone → 真机 rollout(跨数据集,零训练)

smooth800(demo)挖的 milestone 质心直接 assign 到 autonomy rollout(7676 帧,含多次重试):**V_milestone(windowed)复现了与监督 pi0-AE 一致的多段重试结构**(~3000 / ~5000 帧回落重爬,图7);corr(V_ms, pi0-AE)=0.275,corr(pi0-AE, ViVa)=0.418。重试处回落 = **状态触发**的直接证据。

**图7 — rollout 三 value 对比**(洋红 = V_milestone 零训练;绿 = pi0-AE 监督;蓝 = ViVa)

![图7](../../../visualization/cross_episode_recurrence_value/rollout_smooth800milestone_vs_learned_values.png)

**图8 — 四 value 同步视频抽帧**(完整视频 `temp/autonomy_ep0_4model_value_sync.mp4`;第四面板紫阶梯 = recurrence value)

![图8](../../../visualization/cross_episode_recurrence_value/rollout_autonomy_4model_sync_frame.png)
