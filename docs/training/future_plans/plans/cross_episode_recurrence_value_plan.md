...
**结果**:ep37 init **0.99→0.04**、范围 [0.70,1.00]→**[0.00,1.00]**,修复为正常 0→1(图46）。机理:① 硬边界 V[0]=0 绕过橙色视觉误判;② raw 路保判别力;③ 三路冗余对单路失效**结构性免疫**(任一路被坑,其他维度兜底)。**待办**:几何臂掩膜(proprio+标定投影,治本替代颜色 armmask)中长期;V2.3 全量重提 raw 特征。
