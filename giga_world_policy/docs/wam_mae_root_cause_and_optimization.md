...
**Official 终表(200 eps、终 ckpt、episode_report 同协议;ANS 用 T_a=5)**:

| 模型 | @1 | @10 | @24 | @48 | action 延迟 |
|---|---|---|---|---|---|
| **ANS step50000** | 0.0063 | **0.0288** | **0.0574** | 0.0918 | **283ms** |
| ANS soup(46-50k) | 0.0064 | 0.0288 | 0.0586 | 0.0925 | 292ms |
| abs-best step50000(切断) | 0.0053 | 0.0298 | 0.0595 | **0.0916** | 532ms |
...
2. **ANS vs 切断**:200-ep 全集 @48 打平(0.0918 vs 0.0916)、@10/@24 ANS 小胜,
   60-ep 难子集上 ANS 长程优势更明显(0.1245 vs 0.1285)→ **精度 ≥ 切断(难样本更优),
   延迟 283 vs 532ms ≈ −47%**。naive(0.0969)仍为三者最差,Table 6 复现结论不变。
   **复测(同 ckpt 重采样,2026-06-12)**:abs-best 200ep 复测 .0053/.0301/.0580/.0912
   vs 原测 .0053/.0298/.0595/.0916 → **运行间方差 ±0.0005~0.0015**;据此 @24 的 ANS 小胜
   亦落入噪声带,ANS 的确定性优势收敛为「延迟减半 + 难样本长程」;@10 的 −0.001 两次同向,
   可能真实但极小。
