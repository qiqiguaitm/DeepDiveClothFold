...
def get_device_cc(device) -> int:
    """Returns the compute capability of a given torch device if it's a CUDA
    device, otherwise returns 0.
...
class NeighborhoodAttention(nn.Module):
    """Implements 3D neighborhood attention using NATTEN for efficient local
    attention in video."""

    def __init__(self, natten_params: dict):
        super(NeighborhoodAttention, self).__init__()  # noqa: UP008
        self.natten_params = natten_params
        if not isinstance(natten_params, Mapping) or 'window_size' not in natten_params:
            raise ValueError(f'Expected `natten_params` to be a dict with at least keys `window_size`, got {natten_params=}.')

        self.window_size = natten_params['window_size']
        self.stride = 1 if 'stride' not in natten_params else natten_params['stride']
        self.dilation = 1 if 'dilation' not in natten_params else natten_params['dilation']
        self.is_causal = False if 'is_causal' not in natten_params else natten_params['is_causal']
        self.base_size = None if 'base_size' not in natten_params else natten_params['base_size']
        ...
        if (not isinstance(self.dilation, Sequence) or len(self.dilation) != 3) and not isinstance(self.dilation, int):
            raise ValueError(f'Invalid dilation value. Expected an iterable of length 3, or integer, got {self.dilation}.')

        if (not isinstance(self.is_causal, Sequence) or len(self.is_causal) != 3) and not isinstance(self.is_causal, bool):
            raise ValueError(f'Invalid is_causal value. Expected an iterable of length 3, or boolean, got {self.is_causal}.')

        if self.base_size is not None and (not isinstance(self.base_size, Sequence) or len(self.base_size) != 3):
            raise ValueError(f'Invalid base feature map size. Expected an iterable of length 3, or None, got {self.base_size}.')
    ...
    def get_adaptive_parameters(self, window_size, stride, dilation, is_causal, input_shape, base_size=None):
        ...
        if base_size is not None:
            ...
            scaled_window_size = tuple(min(max(2, round(w * s)), x) for w, s, x in zip(window_size, scale, input_shape, strict=False))
            scaled_stride = tuple(min(max(1, round(st * s)), w) for w, s, st in zip(scaled_window_size, scale, stride, strict=False))

            max_dilation = tuple(x // w for x, w in zip(input_shape, scaled_window_size, strict=False))
            scaled_dilation = tuple(min(max(1, round(d * s)), max_d) for d, s, max_d in zip(dilation, scale, max_dilation, strict=False))
    ...
    def forward(
        ...
        if not (q_B_L_H_D.shape == k_B_L_H_D.shape == v_B_L_H_D.shape):
            raise ValueError(f'NATTEN requires QKV shapes to match, got {q_B_L_H_D.shape=}, {k_B_L_H_D.shape=}, {v_B_L_H_D.shape=}.')
        ...
        out = natten.functional.neighborhood_attention_generic(
            query=q,
            key=k,
            value=v,
