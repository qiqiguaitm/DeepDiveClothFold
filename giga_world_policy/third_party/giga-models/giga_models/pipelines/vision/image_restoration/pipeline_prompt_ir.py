...
class PromptIRPipeline(BasePipeline):
    """Pipeline for running PromptIR-based image restoration."""

    def __init__(self, model_path: str) -> None:
        """Initialize the pipeline and load weights.

        Args:
            model_path: Local path to the PromptIR checkpoint file. If missing,
        ...
        self.model.load_state_dict(process_state_dict(state_dict))
        self.model.to(self.dtype)
        self.model.eval()

    def download(self, file_path: str) -> None:
        # download from https://drive.google.com/file/d/1j-b5Od70pGF7oaCqKAfUzmf-N-xEAjYl/view
        download_from_gdrive('1j-b5Od70pGF7oaCqKAfUzmf-N-xEAjYl', file_path)
    ...
    def __call__(self, image: Image.Image) -> Image.Image:
        """Restore the provided image using PromptIR.

        Args:
            image: Input PIL image to be restored.

        Returns:
            A restored PIL image with the original size.
        """
        width, height = image.width, image.height
        image = pad(image)
        image = np.array(image)[:, :, ::-1].copy()
        image = torch.from_numpy(image[None]).permute(0, 3, 1, 2).contiguous() / 255.0
        image = image.to(self.device, dtype=self.dtype)
        with torch.no_grad():
            restored_image = self.model(image)
        restored_image = restored_image.detach().cpu().numpy()[0]
        restored_image = restored_image.transpose(1, 2, 0)
        restored_image = np.clip(restored_image * 255, 0, 255).astype(np.uint8)
        restored_image = Image.fromarray(restored_image[:, :, ::-1])
        restored_image = restored_image.crop((0, 0, width, height))
...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim: int = 128, prompt_len: int = 5, prompt_size: int = 96, lin_dim: int = 192) -> None:
...
def pad(image: Image.Image, part: int = 8) -> Image.Image:
    w, h = image.size
    multiplier_h = int(h / part) + 1 if h % 8 != 0 else int(h / part)
    multiplier_w = int(w / part) + 1 if w % 8 != 0 else int(w / part)
    new_h = multiplier_h * part
    new_w = multiplier_w * part
    new_image = Image.new('RGB', (new_w, new_h), color='black')
