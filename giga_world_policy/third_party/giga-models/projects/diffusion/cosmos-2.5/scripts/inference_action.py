import json
import os
from typing import List

import imageio
import numpy as np
import torch
import torch.distributed as dist
import torch.multiprocessing as mp
import tyro
from accelerate.utils import set_seed
from cosmos2_5.action_utils import get_actions_from_states
from giga_datasets import image_utils
from giga_datasets import utils as gd_utils
from PIL import Image
from torchvision.transforms import InterpolationMode
from torchvision.transforms import functional as F
from tqdm import tqdm

from giga_models import Cosmos25Pipeline
from giga_models.acceleration import get_sequence_parallel_group, initialize_sequence_parallel_group
from giga_models.utils import find_free_port


def _inference(
    device,
    data_path: str,
    save_dir: str,
    transformer_model_path: str,
    text_encoder_model_path: str = None,
    vae_model_path: str = None,
    num_inference_steps: int = 35,
    fps: int = 28,
    num_frames: int = 13,
    height: int = 256,
    width: int = 320,
    seed: int = 1,
    dp_world_size: int = 1,
    dp_rank: int = 0,
    process_index: int = 0,
):
    """Internal function to run inference on a single process.

    Args:
        device: The torch device to run the model on.
        data_path: Path to the JSON file containing inference data.
        save_dir: Directory to save the generated videos.
        transformer_model_path: Path to the pretrained transformer model.
        text_encoder_model_path: Path to the pretrained text encoder model.
        vae_model_path: Path to the pretrained VAE model.
        num_inference_steps: Number of steps for the diffusion process.
        fps: Frames per second for the generated video.
        num_frames: Number of frames to generate in each chunk.
        height: Height of the generated frames.
        width: Width of the generated frames.
        seed: Random seed for reproducibility.
        dp_world_size: World size for data parallelism.
        dp_rank: Rank for data parallelism.
        process_index: The overall index of the current process.
    """
    # Load the inference pipeline
    pipe = Cosmos25Pipeline.from_pretrained(
        transformer_model_path=transformer_model_path,
        text_encoder_model_path=text_encoder_model_path,
        vae_model_path=vae_model_path,
    )
    pipe.to(device, dtype=torch.bfloat16)
    # pipe.set_attn_backend('sage')

    # Load and partition the data for distributed inference
    data_list = json.load(open(data_path, 'r'))
    data_list = gd_utils.split_data(data_list, dp_world_size, dp_rank)
    os.makedirs(save_dir, exist_ok=True)

    # Iterate through the dataset and perform inference
    for n in tqdm(range(len(data_list))):
        set_seed(seed)
        data_dict = data_list[n]
        prompt = data_dict['prompt']
        image_path = data_dict['image']
        states = np.array(data_dict['states'])
        actions = get_actions_from_states(states)

        # Load and preprocess the initial image
        if not os.path.exists(image_path):
            image_path = os.path.join(os.path.dirname(data_path), image_path)
        image = Image.open(image_path)
        image_width, image_height = image.width, image.height
        dst_width, dst_height = image_utils.get_image_size((image_width, image_height), (width, height), mode='area', multiple=16)

        # --- Resize and Crop ---
        if float(dst_height) / image_height < float(dst_width) / image_width:
            new_height = int(round(float(dst_width) / image_width * image_height))
            new_width = dst_width
        else:
            new_height = dst_height
            new_width = int(round(float(dst_height) / image_height * image_width))
        assert dst_width <= new_width and dst_height <= new_height
        x1 = (new_width - dst_width) // 2
        y1 = (new_height - dst_height) // 2
        input_image = F.resize(image, (new_height, new_width), InterpolationMode.BILINEAR)
        input_image = F.crop(input_image, y1, x1, dst_height, dst_width)

        # --- Autoregressive Generation ---
        output_images = []
        input_image_i = input_image
        chunk_size = num_frames - 1
        for i in range(0, len(actions), chunk_size):
            actions_i = actions[i : i + chunk_size]
            real_size = actions_i.shape[0]
            # Pad actions if the last chunk is smaller
            if real_size < chunk_size:
                pad_len = chunk_size - real_size
                pad_shape = [pad_len] + list(actions.shape[1:])
                pad_actions = np.zeros(pad_shape, dtype=actions.dtype)
                actions_i = np.concatenate([actions_i, pad_actions], axis=0)
            actions_i = torch.from_numpy(actions_i).unsqueeze(0)

            # Generate one chunk of frames
            output_images_i = pipe(
                prompt=prompt,
                image=input_image_i,
                guidance_scale=0,
                num_inference_steps=num_inference_steps,
                fps=fps,
                num_frames=num_frames,
                height=height,
                width=width,
                action_latents=actions_i,
                cond_timestep=0,
                seed=seed,
                use_kerras_sigma=False,
                pad_mode='zero',
            )[0]

            # Append generated frames to the list
            output_images_i = output_images_i[:real_size]
            if i == 0:
                output_images.extend(output_images_i)
            else:
                # Skip the first frame as it's the same as the last frame of the previous chunk
                output_images.extend(output_images_i[1:])
            # Set the last generated frame as the input for the next chunk
            input_image_i = output_images[-1]

        # Save the generated video
        if process_index == 0:
            vis_images = []
            for k in range(len(output_images)):
                # Concatenate input and output for visualization
                vis_image = [input_image, output_images[k]]
                vis_image = image_utils.concat_images_grid(vis_image, cols=2, pad=2)
                vis_images.append(vis_image)
            save_path = os.path.join(save_dir, f'{n}.mp4')
            imageio.mimsave(save_path, vis_images, fps=fps)


def _inference_sp(rank, gpu_ids, sp_size, port, kwargs):
    """Worker function for sequence parallel (SP) inference.

    Initializes the distributed process group and then calls the internal
    _inference function.

    Args:
        rank: The rank of the current process.
        gpu_ids: A list of GPU IDs to be used for inference.
        sp_size: The size of the sequence parallel group.
        port: The port for initializing the distributed process group.
        kwargs: A dictionary of arguments to be passed to _inference.
    """
    gpu_id = gpu_ids[rank]
    world_size = len(gpu_ids)
    torch.cuda.set_device(gpu_id)
    device = f'cuda:{gpu_id}'
    dist.init_process_group(
        backend='nccl',
        init_method=f'tcp://127.0.0.1:{port}',
        world_size=world_size,
        rank=rank,
        device_id=torch.device(device),
    )
    initialize_sequence_parallel_group(sp_size)
    sp_group = get_sequence_parallel_group()
    sp_world_size = dist.get_world_size(sp_group)
    sp_rank = dist.get_rank(sp_group)
    dp_world_size = world_size // sp_world_size
    dp_rank = rank // sp_world_size
    assert sp_size == sp_world_size
    _inference(device, dp_world_size=dp_world_size, dp_rank=dp_rank, process_index=sp_rank, **kwargs)


def inference(
    data_path: str,
    save_dir: str,
    transformer_model_path: str,
    text_encoder_model_path: str = None,
    vae_model_path: str = None,
    gpu_ids: List[int] = [0],
    num_inference_steps: int = 35,
    fps: int = 28,
    num_frames: int = 13,
    height: int = 256,
    width: int = 320,
    seed: int = 1,
):
    """Main function to start the inference process.

    This function handles both single-GPU and multi-GPU (via multiprocessing)
    inference.

    Args:
        data_path: Path to the JSON file containing inference data.
        save_dir: Directory to save the generated videos.
        transformer_model_path: Path to the pretrained transformer model.
        text_encoder_model_path: Path to the pretrained text encoder model.
        vae_model_path: Path to the pretrained VAE model.
        gpu_ids: A list of GPU IDs to use for inference.
        num_inference_steps: Number of steps for the diffusion process.
        fps: Frames per second for the generated video.
        num_frames: Number of frames to generate in each chunk.
        height: Height of the generated frames.
        width: Width of the generated frames.
        seed: Random seed for reproducibility.
    """
    kwargs = dict(
        data_path=data_path,
        save_dir=save_dir,
        transformer_model_path=transformer_model_path,
        text_encoder_model_path=text_encoder_model_path,
        vae_model_path=vae_model_path,
        num_inference_steps=num_inference_steps,
        fps=fps,
        num_frames=num_frames,
        height=height,
        width=width,
        seed=seed,
    )
    num_gpus = len(gpu_ids)
    assert num_gpus >= 1
    if num_gpus == 1:
        # Single GPU inference
        _inference(f'cuda:{gpu_ids[0]}', **kwargs)
    else:
        # Multi-GPU inference using multiprocessing
        port = find_free_port()
        mp.start_processes(
            _inference_sp,
            nprocs=num_gpus,
            args=(gpu_ids, num_gpus, port, kwargs),
        )


if __name__ == '__main__':
    tyro.cli(inference)
