...
### 3f. 降级预案(若几何塑形训不出来,按此顺序退)

1. u·z 单调 ranking 训不动 → 退**独立价值头** v(g_t|lang)(~0.2M,仍零查表,只是丢了优雅);
2. MDN 密度作验证不可靠(校准差)→ 退**外挂关系型 verifier** f(G_t,Ĝ_sub,lang)(~0.5M,GHIL-Glue 式,负例=同任务时间反序为主力);
3. 码距达成判定抖动 → 退 gist 余弦 + 滞回,再不行退学习型 reach 头。
   —— 三条退路全部仍满足红线一/二(学习式、零 CRAVE);退的只是 T8 优雅性。
