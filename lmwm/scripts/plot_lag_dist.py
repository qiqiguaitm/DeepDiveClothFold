#!/usr/bin/env python
"""Distribution plot: LMWM (ours, ccenter) vs LaWM prediction time-lag. Overlaid histograms of the
per-frame model_lag (s), with means + design horizons marked, to visualize the difference intuitively.
"""
...
ours = np.load(REPO / "lmwm/outputs/lag_raw_ccenter.npy")
lawm = np.load(REPO / "lmwm/outputs/lag_raw_lawm.npy")

O_HZ, L_HZ = 2.787, 1.66                                                     # dataset horizons
fig, ax = plt.subplots(1, 2, figsize=(13, 4.6))

bins = np.linspace(-2, 6, 49)
for a, title, xr in [(ax[0], "full range", (-2, 6)), (ax[1], "zoom [-1, 3]", (-1, 3))]:
    a.hist(ours, bins=bins, density=True, alpha=0.55, color="#7c3aed", label=f"LMWM ccenter (μ={ours.mean():.2f}s)")
...
fig.suptitle("Prediction time-lag distribution: LMWM (milestone 2.8s) vs LaWM (near-future 1.6s)", fontsize=12)
