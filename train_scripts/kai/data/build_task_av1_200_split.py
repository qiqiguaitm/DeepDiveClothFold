#!/usr/bin/env python3
"""Build Task_AV1_200 (train) + Task_AV1_200_val (held-out val) for the Vertical-Fold-v1
首次基线 (plan: docs/.../plans/pi05_task_av1_vertical_fold_v1_baseline.md §3, §7).

决策 (§7):
  - 选 200 = 按日期顺序取前 200 = 06-11-v2 全 133 + 06-12-v2 前 67.
  - val = 留出 = 06-12-v2 末 45 (第 68 个起, src idx 67..111).
  - 夹爪 = 原始 action (不裁).
  - prompt = "Flatten and fold the cloth. Vertical Fold v1." (B 规范, 修 Flod→Fold + 去尾空格).

机制 (复用 build_kai_vis_merged 已验证套路): 单 chunk-000, 连续重排 episode_index, 视频 symlink 到
源 realpath (不重编码), drop depth 特征 (videos 未同步且下游不用), 写 episodes_stats + info + tasks,
...
SRC_BASE = ROOT / "Task_AV1" / "base"
# date-ordered sources (前者优先)
DATE_ORDER = ["2026-06-11-v2", "2026-06-12-v2"]
CAMERAS = ("top_head", "hand_left", "hand_right")
FPS = 30
PROMPT = "Flatten and fold the cloth. Vertical Fold v1."
N_TRAIN = 200
CHUNK = 0


def list_src_eps():
    """Return date-ordered global list of (date, src_ep_idx, src_dir)."""
    items = []
    for d in DATE_ORDER:
        sd = SRC_BASE / d
...
def write_split(name: str, picks, info_template: dict):
    ...
    for new_ep, (d, src_ep, sd, _succ) in enumerate(picks):
        ...
        for cam in CAMERAS:
            # source dirs are named "<cam>" (raw TOS layout); output uses lerobot key "observation.images.<cam>"
    ...
    # info.json: clone source, drop depth feature, fix counts/chunks
...
def main():
    ...
    print(f"sources (date order): {counts}  total={len(items)}", flush=True)
    if len(items) < N_TRAIN + 1:
        sys.exit(f"FATAL: only {len(items)} ep, need > {N_TRAIN}")

    train_picks = items[:N_TRAIN]
    val_picks = items[N_TRAIN:]
    tc = {d: sum(1 for x in train_picks if x[0] == d) for d in DATE_ORDER}
    vc = {d: sum(1 for x in val_picks if x[0] == d) for d in DATE_ORDER}
    tf = sum(1 for x in train_picks if not x[3])  # success==False count
    vf = sum(1 for x in val_picks if not x[3])
    print(f"train={len(train_picks)} {tc}  (success=False: {tf})", flush=True)
    print(f"val=  {len(val_picks)} {vc}  (success=False: {vf})", flush=True)
    if a.dry_run:
        print("dry-run: nothing written"); return

    info_template = json.loads((SRC_BASE / DATE_ORDER[0] / "meta" / "info.json").read_text())
    train_dst = write_split("Task_AV1_200", train_picks, info_template)
    val_dst = write_split("Task_AV1_200_val", val_picks, info_template)
