...
def run(cache, mine_eps, dt, lam, medw, test_cache, test_ep):
    ...
    cb = [[int(np.argmin(abs(bins - Pk[c])))] for c in order]; pen = lam * np.abs(bins[:, None] - bins[None])
...
fig.tight_layout(); out = REPO / "docs/visualization/cross_episode_recurrence_value/crave_mine_freq_compare.png"
